...
export class UsersRepository {
  ...
  getUserById(id: string): Promise<User> {
    const data = this.usersRepository.findOneBy({ id });
    if (!data) {
  ...
  async getUserByEmail(email: string, includePassword = false): Promise<User | undefined> {
    ...
    // TODO: check if I should remove exception from here
    if (!user) {
      throw new NotFoundException();
    }
